...
export default function TrainingInput(props) {
  ...
  const timesTrainedToday = useSelector(
    (state) => state.intelligence.timesTrainedToday
  );

  const expand = () => {
    setIsExpanded(true);
    Animated.timing(widthAnim, {
      toValue: width - width * 0.02 - height / 60 - 65,
      duration: 200,
      useNativeDriver: false,
    }).start();
    Animated.timing(heightAnim, {
      toValue: 130 + height / 60,
      duration: 200,
      useNativeDriver: false,
    }).start();
  };
  ...
  return (
    <Animated.View
      style={[{ width: widthAnim, height: heightAnim }, styles.expandButton]}
    >
      ...
      <View style={styles.trainingRow}>
        <InventoryImage
          ...
          onUse={(dispatch) => {
            clearTimeout(activeGuess);
            setActiveGuess(0);
            SHOP_ITEM_INFO.apple.onUse(dispatch, trick, trickAttempt, true);
            dispatch(guess(null, null));
          }}
        />
        <InventoryImage
          ...
          onUse={(dispatch) => {
            clearTimeout(activeGuess);
            setActiveGuess(0);
            SHOP_ITEM_INFO.milkBone.onUse(dispatch, trick, trickAttempt, true);
            dispatch(guess(null, null));
          }}
        />
        <InventoryImage
          ...
          onUse={(dispatch) => {
            clearTimeout(activeGuess);
            setActiveGuess(0);
            SHOP_ITEM_INFO.peanutButter.onUse(
              dispatch,
              trick,
              trickAttempt,
              true
