...
export default function CareerOption(props) {
  ...
  const cleanlinessStars = useSelector(
    (state) => state.cleanliness.cleanlinessStars
  );
  const starVals = {
    love: 0,
    hunger: hungerStars,
  ...
  return (
    <View
      style={[styles.optionWrapper, { backgroundColor: props.primaryColor }]}
    >
      <TouchableOpacity
      ...
      <View>
        <Text style={[styles.jobTitle, styles.font]}>
          {props.displayString}
        </Text>
      </View>
      ...
      <View style={styles.barStatsWrapper}>
        <View style={styles.barStatsArea}>
          {Object.keys(props.barVals).map((e) => {
            let icons = new Array(
              showUserStars ? starVals[e] : props.barVals[e]
            );
            for (let i = 0; i < icons.length; i++) {
              console.log(e + String(i));
              icons[i] = (
                <Icon
                  type="FontAwesome"
                  name="star"
            ...
            return (
              <View style={styles.barStatLine}>
                <View style={styles.barStatTextWrapper} key="">
                  <Text style={[styles.barStat, styles.font]}>{e + ':'}</Text>
                </View>
                <View style={styles.barStatIconWrapper} key="iconWrapper">
                  {icons}
                </View>
              </View>
            );
      ...
      <View>
        ...
        >
          <Text style={[styles.font, styles.buttonFont]}>
            {'Become a ' + props.displayString + ' Dog!'}
          </Text>
        </TouchableOpacity>
      </View>
      <Modal
      ...
      >
        <View style={styles.modal}>
          ...
          >
            ...
            >
              <Text style={[styles.buttonFont, styles.font]}>Close</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
