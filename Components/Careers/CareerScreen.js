...
import {
  reduxAndFirebaseAddCoins,
  reduxAndFirebaseReduceShift,
  reduxAndFirebaseStartShift,
...
function CareerBackground(props) {
  ...
  const expectedShiftEnd = useSelector(
    (state) => state.career.expectedShiftEnd
  );
    ...
    <View style={styles.pageContainer}>
      {!animalIsShown && (
        <View style={styles.awayMessageContainer}>
          ...
          <TouchableOpacity
            style={styles.reduceShiftButton}
            onPress={() =>
              reduxAndFirebaseReduceShift(dispatch, expectedShiftEnd)
            }
          >
      ...
      <>
        <TouchableOpacity style={styles.busButton} onPress={fadeIn}>
          ...
          >
            <Image
              style={styles.workBus}
              source={require('../../assets/workBus.png')}
            ></Image>
            <View style={styles.workIcon}>{careerInfo.icon}</View>
          </Animated.View>
        </TouchableOpacity>
        <CareerWorkMenu
          fadeOut={fadeOut}
...
export default function CareerScreen(props) {
  ...
  const [showCareerSelection, setShowCareerSelection] = useState(
    careerId === null
  );
  ...
  return (
    <OverlayContainer
      front={
        showCareerSelection ? (
          <SelectCareerScreen setShowCareerSelection={setShowCareerSelection} />
        ) : null
      }
      behind={
        <CareerBackground setShowCareerSelection={setShowCareerSelection} />
      }
    />
  );
}
...
const styles = StyleSheet.create({
  ...
  workIcon: {
    position: 'absolute',
    top: WORK_BUS_HEIGHT * 0.55,
    left: WINDOW_WIDTH * 0.6,
  },
  awayMessage: {
    fontSize: 30,
    textAlign: 'center',
