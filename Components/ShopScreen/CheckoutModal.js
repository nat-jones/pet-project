...
export default function CheckoutModal(props) {
  ...
  const getItemsInCart = () => {
    ...
    let summary = items.reduce((acc, e) => {
      if (shoppingCart[e] > 0) {
        let itemInfo = {
          ...SHOP_ITEM_INFO[e],
          quantity: shoppingCart[e],
          lineNumber: acc.length + 1,
        };
        acc.push(itemInfo);
      }
      return acc;
  ...
  const getCartCost = () => {
    let items = Object.keys(shoppingCart);
    let cartCost = items.reduce((acc, e) => {
  ...
  const purchase = async () => {
    ...
    if (coins >= cartCost) {
      await reduxAndFirebasePurchaseCart(
        dispatch,
        inventory,
        shoppingCart,
        cartCost,
