...
export const reduxAndFirebaseSetHunger = async (
  dispatch,
  { lastFed, timesFedToday, hungerStars, lastCheckIn, didMisfeed, time }
) => {
...
export const reduxAndFirebaseSetExercise = async (
  dispatch,
  {
    timesExercisedToday,
    lastExercised,
...
export const reduxAndFirebaseSetCleanliness = async (
  dispatch,
  {
    timesCleanedToday,
    lastCleaned,
...
export const reduxAndFirebaseFeedPet = async (
  dispatch,
  { lastFed, didMisfeed, timesFedToday, time }
) => {
  let data = {
    lastFed: time,
...
export const reduxAndFirebaseExercisePet = async (
  dispatch,
  { lastExercised, didMisexercise, timesExercisedToday, time }
) => {
  let data = {
    lastExercised: time,
    timesExercisedToday: timesExercisedToday + 1,
    didMisexercise: checkHoursSince(time, lastExercised, 2)
      ? didMisexercise
      : true,
  };
...
export const reduxAndFirebaseCleanPet = async (
  dispatch,
  { lastCleaned, didMisclean, timesCleanedToday, time }
) => {
  let data = {
    lastCleaned: time,
...
const checkDaysSince = (lastCared, curTime, threshold) =>
  daysSince(lastCared, curTime) >= threshold;
const checkHoursSince = (lastCared, curTime, threshold) =>
  hoursSince(curTime, lastCared) >= threshold;
...
export const reduxAndFirebaseAddCoins = async (dispatch, lastShiftType) => {
  ...
  let coins = Store.getState().coins;
  ...
  let stars = Object.keys(careerBarVals).reduce((acc, e) => {
  ...
  updateCoins(coins + payment);
  dispatch(addCoins(payment, 'coins'));
};

export const reduxAndFirebasePurchaseCart = async (
  dispatch,
  inventory,
  shoppingCart,
  cartCost,
...
export const reduxAndFirebaseLearn = async (
  dispatch,
  trainingData,
  command,
  attempt,
  ...
  trainingData = Object.keys(trainingData).reduce((acc, e) => {
    acc[e] = { ...trainingData[e] };
    return acc;
  }, {});
  if (!(command in trainingData)) {
    trainingData[command] = { ...TRICK_DEFAULTS };
  }
  trainingData[command][attempt] += outcome ? 0.25 : -0.25;

  if (trainingData[command][attempt] < 1) {
    return;
  } else {
    await firebaseLearn(trainingData, timesTrainedToday);
    dispatch(train());
    dispatch(setTrainingData(trainingData, timesTrainedToday));
  }
};

const addCartload = (cartLoad, inventory) => {
  return Object.keys(cartLoad).reduce((acc, e) => {
    acc[e] += cartLoad[e];
    return acc;
  }, inventory);
};
